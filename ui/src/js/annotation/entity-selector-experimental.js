...
export class EntitySelectorExperimental extends TatorElement {
  constructor() {
    super();
  ...
  _emitSelection(byUser, composed, goToEntityFrame) {
    ...
    if (this._timeKeeper.getNumberFrames()) {
      ...
      if (endFrameCheck === false) {
        this._data[index].frame = this._timeKeeper.getNumberFrames();
      }
      else if (endFrame > -1) {
...
customElements.define("entity-selector-experimental", EntitySelectorExperimental);
