...
export class AttributePanelExperimental extends TatorElement {
  constructor() {
    super();
      ...
      if (attrType.style != null) {
        if (attrType.style.includes("attr_group")) {
          ...
          if (attrType.style.includes("minimized")) {
            this._toggleAttributes(groupInfo.infoDiv);
            this._toggleChevron({target: labelDiv});
          }
        }
      }
    }
...
if (!customElements.get("attribute-panel-experimental")) {
  customElements.define("attribute-panel-experimental", AttributePanelExperimental);
}
