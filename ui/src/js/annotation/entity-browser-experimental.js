...
export class EntityBrowserExperimental extends TatorElement {
  constructor() {
    super();
  ...
  _getGlobalFrame(elem, isSingleMedia) {
    if (isSingleMedia) {
    ...
    for (const group in groups) {
      if (this._dataType.isLocalization) {
        groups[group].sort((item_a, item_b) => {
          if (this._getGlobalFrame(item_a, true) === this._getGlobalFrame(item_b, true)) {
            return item_a.id - item_b.id;
          }
          return this._getGlobalFrame(item_a, true) - this._getGlobalFrame(item_b, true);
        });
        ...
        groups[group].sort((item_a, item_b) => {
          if (this._getGlobalFrame(item_a) === this._getGlobalFrame(item_b)) {
            return item_a.id - item_b.id;
          }
          return this._getGlobalFrame(item_a) - this._getGlobalFrame(item_b);
        });
      }
      if (group in this._selectors) {
        ...
        const selector = document.createElement("entity-selector-experimental");
        selector.timeKeeper = this._timeKeeper;
  ...
  frameChange(globalFrame) {
    if (this._dataType.isTrack) {
      for (let groupId in this._attributes) {
        this._attributes[groupId].setFrame(globalFrame);
      }
    }
  }
...
customElements.define("entity-browser-experimental", EntityBrowserExperimental);
