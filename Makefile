...
webpack: $(TATOR_JS_MODULE_FILE)
	...
	cd ui && npm install && $(HOME)/.local/bin/poetry run python make_index_files.py && npm run build
else
webpack: $(TATOR_JS_MODULE_FILE)
	@echo "Building webpack bundles for development, because USE_MIN_JS is false"
	cd ui && npm install && $(HOME)/.local/bin/poetry run python make_index_files.py && npm run buildDev
endif
